"""
Experiment: Only include admins with exactly 30 recent calls (not more, not less) in ranking.
"""
import pandas as pd
import numpy as np
import sys
...
def run_experiment():
    ...
    for admin_id in admin_ids:
        admin_calls = call_data[call_data['admin_id'] == admin_id].sort_values('created_at', ascending=False)
        num_calls = len(admin_calls)
        if num_calls < MIN_CALLS:
            continue  # Only include admins with at least MIN_CALLS calls
        admin_calls = admin_calls.head(MAX_CALLS)  # Always use most recent 10
    ...
    if not results:
        print(f"No admins found with at least {MIN_CALLS} calls.")
        return
    df = pd.DataFrame(results)
    df = df.sort_values('exp_lambda_score', ascending=False).reset_index(drop=True)
    df['rank'] = df.index + 1
    out_base = os.path.join(os.path.dirname(__file__), f'min_calls_{MIN_CALLS}_admin_rankings')
    out_xlsx = out_base + '.xlsx'
    out_csv = out_base + '.csv'
    df.to_excel(out_xlsx, index=False)
    df.to_csv(out_csv, index=False)
    print(f"\nMin-calls experiment rankings saved to: {out_xlsx} and {out_csv}\n")
    print(df[['rank','admin_name','exp_lambda_score','cr50','r50','cdt50','lr1m','num_calls']].head(10))
